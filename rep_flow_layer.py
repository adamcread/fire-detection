...
class FlowLayer(nn.Module):
    def __init__(self, channels=1, bottleneck=32, params=[1,1,1,1,1], n_iter=10):
        ...
        # applied batch normalisation over a 5D input (a mini-batch of 3D inputs with additional channel dimensions) 
        ...
        # required training or not? 
        # p3 of paper for reasoning
        if params[0]:
    ...
    # normalise image for better learning
    def norm_img(self, x):
        mx = torch.max(x)
        mn = torch.min(x)
        x = 255*(x-mn)/(mn-mx)
    ...
    def forward(self, x):
        residual = x[:,:,:-1] # 0 , ..., t-1
        x = self.bottleneck(x) # bottleneck resuls
        inp = self.norm_img(x) # normalise results
        x = inp[:,:,:-1] # 0, ..., t-2
        y = inp[:,:,1:] # 1, ..., t-1
        ...
        for _ in range(self.n_iter):
            # iterate towards true optical flow

            # u1 = u_x
            # u2 = u_y
            # calculate rho and convert to long?
            ...
            # first condition => if rho < -lambda * theta|gradF_2|^2
            mask1 = (rho < -l_t*grad).detach()
            v1[mask1] = (l_t * grad2_x)[mask1] # apply condition 1 to x
            v2[mask1] = (l_t * grad2_y)[mask1] # apply condition 1 to y
            
            # second condition => if rho > lambda*theta|gradF_2|^2
            mask2 = (rho > l_t*grad).detach()
            v1[mask2] = (-l_t * grad2_x)[mask2] # apply condition 2 to x
            v2[mask2] = (-l_t * grad2_y)[mask2] # apply condition 2 to y

            # print("mask1", mask1)
            # print("mask2", mask2)

            # third condition => otherwise
            # mask1 false, mask2 false, grad big enough to be valid
            # using bitwise operator True => 1 => False
        ...
        flow = self.bn(flow)
        # add flow information into 0, ..., t-1
